...
def parse_auth_log(file_path):
    ...
    # try to detect the log's year from its modification time
    ...
    with open(file_path, "r") as f:
        for line in f:
            ...
            if match:
                ...
                # add the detected year to avoid DeprecationWarning
                full_timestamp_str = f"{file_year} {timestamp_str}"
                try:
                    timestamp = datetime.strptime(full_timestamp_str, "%Y %b %d %H:%M:%S")
                except ValueError:
                    # fallback in case of weird leap day / year mismatch
